...
public class RangeViewer {
	...
	private void fillRange(Robot robot, int range, Graphics g) {
		...
		for (int x = 0; x <= rangePos; x++) {
			...
			if (robot.getName().equals("P1")) {
				g.setColor(new Color(255, 0, 0, 50));
			} else {
				g.setColor(new Color(0, 255, 0, 50));
			}
			
			g.fillRect((robotCol - rangePos + decalage) * rowWid, (robotLine - (rangePos - x + 1)) * rowHt,
					largeur * rowWid, rowHt);
			if (x != rangePos) {
				g.fillRect((robotCol - rangePos + decalage) * rowWid, (robotLine + (range - x)) * rowHt,
						largeur * rowWid, rowHt);
			}
		}
